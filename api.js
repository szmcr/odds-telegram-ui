...
const loadOdds = async (leagues) => {
  ...
  for (let index = 0; index < leagues.length; index++) {
    ...
    try {
      const res = await fetch(`https://api.the-odds-api.com/v4/sports/${leagues[index]}/odds/?regions=us&oddsFormat=american&markets=h2h,spreads,totals&bookmakers=betus&apiKey=8f8be8b7a86adaaba12d6e8d4645326a`);
      if (res.status === 200) {
